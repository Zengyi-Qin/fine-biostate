...
from generic_optimizer import build_train_op
import numpy as np
...
class Trainer(object):
    ...
    def kurtosis_loss(self, output_tf, label_tf):
        ...
        for cls in range(self.num_classes):
            ...
            true_distribution = tf.random_uniform(
                                ...
                                minval=0,
                                maxval=1)
            loss = tf.abs(self.forth_order(masked_output) * self.square_var(true_distribution) - \
                          self.forth_order(true_distribution) * self.square_var(masked_output))
            if total_loss == None:
                total_loss = loss
            else:
                total_loss = total_loss + loss
    ...
    def regression_loss(self, output_tf, label_tf, alpha=0.0, beta=0.5):
        ...
        diff_smooth_l1 = tf.sqrt(tf.square(tf.nn.relu(diff - alpha)) + beta ** 2) - beta
    ...
    def accuracy(self, output_np, label_np):
        return np.mean(np.argmax(output_np, axis=1) == np.argmax(label_np, axis=1))
    ...
    def train(self, batch_size, steps):
        ...
        net.build(signal_tf)
        reg_loss = tf.reduce_mean(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)) * 0.1

        if self.use_fine_grained:
            regression_loss = self.regression_loss(net.output_regression, label_tf, 
                                                   alpha=0.0, beta=0.0)
            ...
            regression_loss = self.regression_loss(net.output_regression, 
                                                   ...
                                                   alpha=0.2, beta=0.1)
            kurtosis_loss = self.kurtosis_loss(net.output_regression, label_tf) * 10.0
            mmd_distribution_loss = self.mmd_loss(net.output_regression, label_tf) * 0.03
            ...
            regression_loss = tf.reduce_mean(tf.square(net.output_regression - 
                                                       tf.floor(label_tf) - 0.5))
            kurtosis_loss = tf.constant(0.0, dtype=tf.float32)
            mmd_distribution_loss = tf.constant(0.0, dtype=tf.float32)
        elif self.regression_type == 'imm':
            regression_loss = self.immediate_threshold_loss(net.output_regression, 
                                                       tf.floor(label_tf) + 0.5)
            kurtosis_loss = tf.constant(0.0, dtype=tf.float32)
            mmd_distribution_loss = tf.constant(0.0, dtype=tf.float32)
            ...
            regression_loss = self.all_threshold_loss(net.output_regression,
                                                       tf.floor(label_tf) + 0.5)
            kurtosis_loss = self.kurtosis_loss(net.output_regression, label_tf) * 10.0
            mmd_distribution_loss = self.mmd_loss(net.output_regression, label_tf) * 0.01
        ...
        with tf.Session(config=config) as sess:
            ...
            for step in range(steps):
                learning_rate_curr = learning_rate_basic / (1+np.exp((step - steps//2)/(steps//4)))
                signal_np_train, label_np_train = self.get_data('train', batch_size)
                feed_dict = {signal_tf: signal_np_train,
                             label_tf: label_np_train,
                             kurtosis_weight: self.get_kurtosis_weight(step, steps),
                             is_training: True,
                ...
                if step%10 == 0:
                    error_np = self.regression_error(output_reg_np_train, label_np_train)
                    print('completed: %.2f%%  loss: %.2f  regression: %.2f  kurtosis: %.2f  distribution: %.2f  error: %.2f'% \
                         (step*100.0/steps, loss_np, regression_loss_np, kurtosis_loss_np,  
                          mmd_distribution_loss_np, error_np))

                if step%100 == 0:
                    signal_np_test, label_np_test = self.get_data('test', batch_size)
                    num_runs = int(math.ceil(signal_np_test.shape[0] * 1.0 / batch_size))
                    ...
                    for i in range(num_runs):
                        feed_dict = {signal_tf: signal_np_test[i * batch_size: (i+1) * batch_size],
                                     ...
                                     is_training: False}
                        [output_curr] = sess.run([net.output_regression], feed_dict=feed_dict)
                        output_regression_test.append(output_curr)
                    output_reg_np_test = np.concatenate(output_regression_test, axis=0)
                    mae = self.regression_mae(output_reg_np_test, label_np_test)
                    if min_mae > mae:
                        min_mae = mae

                    rmse = self.regression_rmse(output_reg_np_test, label_np_test)
                    if min_rmse > rmse:
                        min_rmse = rmse

                    prec = self.precision(output_reg_np_test, label_np_test, 0.5)
                    if prec > max_prec:
                        max_prec = prec
                    ...
                    print('test mae: %.4f, rmse: %.4f, prec: %.4f, inv: %.4f'%(min_mae, min_rmse, max_prec, min_inv))
                    print('------------------')
                if (step + 1)%3000 == 0 or (step + 1) == steps:
                    saver.save(sess, './runs/model_step_{}.ckpt'.format(step+1))
        return 
