...
class SurfaceNet(object):
    ...
    def build(self, x):
        ...
        with tf.variable_scope('feature_space'):        
            ...
            #block A
            self.convAL1 = self.central_pool(self.conv_layer(self.poolSTL_S4, 3, 2, 128, 256, 'convA1', True, True))
            self.convAL2 = self.conv_layer(self.convAL1, 1, 1, 256, 64, 'convA2', False, False)
            self.convAL3 = self.conv_layer(self.convAL2, 3, 1, 64, 256, 'convA3')
            self.poolAL_S16 = self.avg_pool(self.convAL3, 3, 2, 'poolAL_S16')
                       
            #block B
            self.convBL1 = self.central_pool(self.conv_layer(self.poolAL_S16, 3, 1, 256, 256, 'convB1', True, True))
            self.convBL2 = self.conv_layer(self.convBL1, 1, 1, 256, 64, 'convB2', False, False) + \
                       self.avg_pool(self.convAL2, 3, 2,'poolBL2')
            self.convBL3 = self.conv_layer(self.convBL2, 3, 1, 64, 256, 'convB3')
            self.poolBL_S32 = self.avg_pool(self.convBL3, 3, 2, 'poolBL_S32')
        
            #block C
            self.convCL1 = self.central_pool(self.conv_layer(self.poolBL_S32, 3, 1, 256, 256, 'convC1', True, True))
            self.convCL2 = self.conv_layer(self.convCL1, 1, 1, 256, 64, 'convC2', False, False) + \
                       self.avg_pool(self.convBL2, 3, 2, 'poolCL2')
            self.convCL3 = self.conv_layer(self.convCL2, 3, 1, 64, 256, 'convC3')
            self.poolCL_S64 = self.max_pool(self.convCL3, 'poolCL_S64')

            self.convGL1 = self.conv_layer(self.poolCL_S64, 3, 1, 256, 256, 'convG1')
            self.convGL2 = self.conv_layer(self.convGL1, 3, 1, 256, 256, 'convG2')
            self.convGL3 = self.conv_layer(self.convGL2, 3, 1, 256, 1, 'convG3', False, False)
            self.output_regression = tf.reshape(tf.reduce_mean(self.convGL3, axis=1), [-1])
    ...
    def central_pool(self, bottom):
        pool_1 = tf.nn.avg_pool(bottom, ksize=[1, 2, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
        pool_2 = tf.nn.avg_pool(pool_1, ksize=[1, 2, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
        return pool_2

    def conv_layer(self, bottom, ksize, stride, in_channels, out_channels, name, batch_norm=True, relu=True):
        with tf.variable_scope(name):
            filt, conv_biases = self.get_conv_var(ksize, in_channels, out_channels, name)
            conv = tf.nn.conv2d(bottom, filt, [1, stride, 1, 1], padding='SAME')
            if batch_norm:
    ...
    def lstm_layer(self, bottom, time_steps, in_size, out_size, name):
        with tf.variable_scope(name):
            ...
            lstm_outputs, state = tf.nn.dynamic_rnn(cell, tf.reshape(bottom, [-1, time_steps, in_size]), dtype=tf.float32)
            lstm_outputs = tf.transpose(lstm_outputs, [1, 0, 2])
            lstm_last = tf.gather(lstm_outputs, int(lstm_outputs.get_shape()[0]) - 1)
    ...
    def get_conv_var(self, filter_size, in_channels, out_channels, name, reg=1e-2):
        ...
        filters = tf.get_variable(name=name+'filters', shape=shape, initializer=initializer, regularizer=regularizer)
        biases = tf.get_variable(name=name+'biases', shape=[out_channels], initializer=initializer, regularizer=regularizer)
    ...
    def get_fc_var(self, in_size, out_size, name, reg):
        ...
        weights = tf.get_variable(name=name+'weights', shape=shape, initializer=initializer, regularizer=regularizer)
        biases = tf.get_variable(name=name+'biases', shape=[out_size], initializer=initializer, regularizer=regularizer)
